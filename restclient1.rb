...
module MyRestClientTest
  class Test
    ...
    def get_base_info
      ...
      base_buildings_info.each do |building| 
        puts building["id"]
        puts building["type"]
      end
    end
    ...
    def buy_item(item_id)
      ...
      item_info = JSON.load(response.body).values_at("item")[0]
      p item_info.values_at("display_name")
    end
    ...
    def build_new_buildings(x,y, building_type)
      response = RestClient.post "#{@phoenix_api_url}/bases/#{@base_id}/buildings/?player_id=#{@player_id}&token=#{@player_token}&building%5bx%5d=#{x}&building%5by%5d=#{y}&building%5bbuilding_type%5d=Buildings%3a%3a#{building_type}&_method=POST@instant=true", {:params => {}}
    ...
    def destory_building(building_id)
      ...
      p building_info = JSON.load(response.body)['success']
    end
...
t.get_base_info
# p t.build_new_buildings(-1700, -2200, "FuelDepot")[:building_id]
# t.destory_building(126321)
